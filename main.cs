...
class MainClass 
{
  ...
  public static Queue<studentas> galvociai = new Queue<studentas>();
    ...
    while (tikrinimas)
    {
      ...
      if (c == "1")
      {
        try
        {
          ...
          Queue<double> balai = new Queue<double>();
          for (int i = 2; i < whitesp.Length - 1; i++)
          {
            balai.Enqueue(double.Parse(whitesp[i]));
          } 

          studentas laik = new studentas(vardas, pavarde, rezEgz, balai);
          studentai.Enqueue(laik);
          if (laik.galutinis < 5)
          {
            vargsiukai.Enqueue(laik);
          }
          if (laik.galutinis >= 5)
          {
            galvociai.Enqueue(laik);
          }
          stopwatch.Stop();
